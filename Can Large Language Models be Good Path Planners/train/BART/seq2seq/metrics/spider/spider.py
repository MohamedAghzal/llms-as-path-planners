...
def compute_exact_match(predictions, references):
    count = sum([int(x==y) for x, y in zip(predictions, references)])
...
class Spider(datasets.Metric):
    ...
    def _compute(self, predictions, references):
        t_references = [r['label'] for r in references]
        # pdb.set_trace()
        if self.config_name == "exact_match" or self.config_name == "both":
            exact_match = {'exact_match': compute_exact_match(predictions, t_references)} #compute_exact_match_metric(predictions, references)
        else:
            raise NotImplementedError('Specified Metric is not supported in the corret implementation.')
        return {**exact_match}
